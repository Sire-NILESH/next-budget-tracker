...
function EditCategoryDialog({
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: async (result: EditCategoryResponseType) => {
      const updatedCategoryData = result[0];
      ...
      toast.success(
        `Category ${updatedCategoryData.name} edited successfully 🎉`,
        {
          id: "edit-category",
        }
      );

      successCallback(updatedCategoryData);
  ...
  const onSubmit = useCallback(
    ...
    [category.icon, category.name, category.type, mutate]
  );
